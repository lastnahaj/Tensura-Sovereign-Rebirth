...
- Great Sage requires the player to actually possess Great Sage; verify the exact configuration key during assembly.
...
A privately supplied quest archive was reviewed only for high-level campaign organization, pacing, chapter dependencies, and shared reward-table patterns. TSR's quest text, identifiers, rewards, detection logic, and serialized files will be created independently against the assembled pack. The private archive and its source files are not part of this repository.
