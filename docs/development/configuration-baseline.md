...
- FTB Quests uses a 20-tick detection interval and team-shared rewards. The eight authored act chapters use custom TSR backgrounds and direct project-wiki links. With the active MineColonies quest integration, the tested server loads 9 chapters and 140 quests.
...
- The pack-owned TSR Client Stability JAR protects the existing `options.txt` mappings during NeoForge's early option load and reloads the preserved values after late mod key registration. An FTB Chunks map-key change survived two complete expanded-client restarts.
