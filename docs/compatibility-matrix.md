...
| FTB Chunks | Xaero's Minimap / World Map | FTB Chunks x Xaero's Map Compat | Explicit Integration | 1.1.4 | Verified graphical startup | Official NeoForge 1.21.1 file `8103626` (`1357724`) is active in the 209-JAR client profile; the expanded graphical smoke and two restart passes succeed. |
| FTB Essentials | TAB | Server administration and player list | Server profile | 2101.1.10 / 5.5.0 | Verified | TAB is server-only and dual-platform pinned; the 175-JAR server passes cold startup, save, shutdown, and warm restart with both active. |
...
| Tensura Skill Trainer | Player-managed optional extension | Distribution boundary | Optional | 2.0.5 | USER-OPTIONAL-NOT-SHIPPED | Deliberately absent from client and server packages; players may add it independently. |
| C2ME | Dedicated-server shutdown | Stability boundary | Optimization | 0.4.0-alpha.0.120 | DEFERRED-BLOCKED | Excluded after a clean shutdown hang; no replacement is active. |
...
| Tensura Skill Books | FTB Quests | Controlled skill rewards | Authored quest integration | 2.2.1 / 2101.1.34 | Under Validation | Natural loot injection is empty; the tested runtime loads 9 chapters and 140 quests, while reward balance still requires gameplay validation. |
...
| SlimeThrone | FTB Quests | Repeatable prestige versus authored campaign | Separated ownership | 2.1.2.1 / 2101.1.34 | Beta campaign implemented | Eight branded authored act chapters plus the MineColonies integration chapter load successfully; deeper reward and pacing playtests remain. |
...
| MineColonies | Lunara Colonies | Azurecrest style pack | External addon | 1.21.1.28 | Active private beta | Expanded server cold/warm and client graphical startup pass; the selected exact artifact remains a public-Modrinth source exception. |
| MineColonies | JadeColonies | Jade colony overlays | External addon | 1.21.1-1.6.0 | Active private beta | Jade is installed on both sides to satisfy the integration dependency; cold/warm server and client startup pass. Public Modrinth source remains pending. |
| MineColonies | Minecolonies Questline | FTB Quests installer chapter | External addon | 1.21.1-1.1.0 | Active private beta | The full runtime loads 9 chapters and 140 quests. Public Modrinth source remains pending. |
| MineColonies | Stylecolonies | Blueprint style sets | External addon | 1.15.58 / 1.21.1 | Active private beta | Server structure-pack discovery and clean/warm startup pass. Public Modrinth source remains pending. |
