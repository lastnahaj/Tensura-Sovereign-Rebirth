---
title: Phaser
description: '"When a man learns to love, he must also bear the risk of carrying hate"'
tags:
- Unique_Skills
- Abilities_changed_in_1.21.1
- Abilities_changed_in_1.19.2
---

# Phaser

<span class="reference-badge">TR Mysticism reference</span> <span class="reference-category">Unique Skills</span>

<section class="reference-overview reference-theme-abilities">
<figure class="reference-overview-media reference-overview-media--source"><img src="../../../../assets/upstream/mysticism/skills/phaser-71dbe5037f.png" alt="Phaser source icon" loading="eager" decoding="async"><figcaption><a href="https://trmysticism.wiki.gg/wiki/File:Phaser.png">Phaser.png · CC BY-SA 4.0</a></figcaption></figure>
<div class="reference-overview-copy">
<p class="reference-eyebrow">At a glance</p>
<p>&quot;When a man learns to love, he must also bear the risk of carrying hate&quot;</p>
<nav class="reference-quick-jumps" aria-label="Article sections">
<a href="#how-to-obtain">How to obtain</a>
<a href="#Usage">Usage</a>
<a href="#Version_history">Version history</a>
</nav>
<div class="reference-reading-controls" role="group" aria-label="Article reading mode">
<button type="button" class="reference-mode-button is-active" data-reference-mode="overview" aria-pressed="true">Overview</button>
<button type="button" class="reference-mode-button" data-reference-mode="full" aria-pressed="false">Expand all</button>
</div>
</div>
</section>

<!-- skill-catalogue:start -->
<nav class="skill-category-nav" aria-label="Skill directory"><a href="../../../../tensura-reference/skills/unique/">← Browse Unique Skills</a></nav>
<section class="skill-obtainment" aria-labelledby="how-to-obtain"><p class="reference-eyebrow">Unique Skills · Pinned pack inventory</p><h2 id="how-to-obtain">How to obtain</h2><p>Source-described obtainment methods:</p><ul><li><strong>Other:</strong>
Reincarnation/Reroll</li></ul><p class="skill-evidence-note">An obtainment method is separate from a skill’s MP cost. Check all conditions; a listed route is not a guaranteed starting roll.</p></section>
<!-- skill-catalogue:end -->

<div class="tensura-reference-article">
<div class="mw-content-ltr mw-parser-output" dir="ltr" lang="en"><p>"When a man learns to love, he must also bear the risk of carrying hate"
</p>
<div class="druid-infobox druid-container noexcerpt druid-container-skill" id="druid-container-1"><div><div class="druid-title">Phaser</div></div><div class="druid-section-container"><div class="druid-main-image"><a class="image" href="https://trmysticism.wiki.gg/wiki/File:Phaser.png"><img alt="Phaser.png" data-file-height="512" data-file-width="512" decoding="async" height="256" loading="lazy" src="../../../../assets/upstream/mysticism/skills/phaser-71dbe5037f.png" width="256"/></a></div></div><div class="druid-section-container"><div data-druid-section="Information"><div class="druid-section druid-section-Information">Information</div></div><div class="druid-row druid-row-Type" data-druid-section-row="Information"><div class="druid-label druid-label-Type">Type</div><div class="druid-data druid-data-Type druid-data-nonempty">Unique Skill</div></div><div class="druid-row druid-row-ObtainCost" data-druid-section-row="Information"><div class="druid-label druid-label-ObtainCost">Obtain Cost</div><div class="druid-data druid-data-ObtainCost druid-data-nonempty">
72K</div></div><div class="druid-row druid-row-PointstoMaster" data-druid-section-row="Information"><div class="druid-label druid-label-PointstoMaster">Points to Master</div><div class="druid-data druid-data-PointstoMaster druid-data-nonempty">
1000</div></div><div class="druid-row druid-row-Passive" data-druid-section-row="Information"><div class="druid-label druid-label-Passive">Passive</div><div class="druid-data druid-data-Passive druid-data-nonempty">
True, Toggle</div></div></div><div class="druid-section-container"><div data-druid-section="Evolution"><div class="druid-section druid-section-Evolution">Evolution</div></div><div class="druid-row druid-row-Next" data-druid-section-row="Evolution"><div class="druid-label druid-label-Next">Next</div><div class="druid-data druid-data-Next druid-data-nonempty">
[[]]</div></div></div><div class="druid-section-container"><div data-druid-section="Obtaining"><div class="druid-section druid-section-Obtaining">Obtaining</div></div><div class="druid-row druid-row-Other" data-druid-section-row="Obtaining"><div class="druid-label druid-label-Other">Other</div><div class="druid-data druid-data-Other druid-data-nonempty">
Reincarnation/Reroll</div></div></div></div>

<h2><span class="mw-headline" id="Usage">Usage</span></h2>
<h3><span class="mw-headline" id="Passive">Passive</span></h3>
<ul><li>[Passive, Toggle] Eye of Insight - When toggled on, gain the effects of Thought Acceleration and Magic Sense. Drains 50 Magicules every 5 seconds that this is toggled on.</li></ul>
<ul><li>[Passive, True] Overuse:</li></ul>
<p>Using abilities from this skill will build up a counter. If the counter hits 10, it will strain the user’s eyes, putting the entire skill on cooldown for 180 seconds while the user recharges.
</p><p>Resting for 15 seconds after an ability usage will reset the counter to zero.
</p>
<ul><li>[Passive, True] Intangibility:</li></ul>
<p>The next attack you take will be dodged with 100% chance. For 5 seconds, you are intangible and immune to all attacks, as they simply pass through you. This includes projectiles. However, this passive goes on a 15 second cooldown after.
</p><p>Will build up the counter if this is triggered.
<br/>
</p>
<h3><span class="mw-headline" id="Active">Active</span></h3>
<ul><li>[Active, Held/Press] Ejection:</li></ul>
<p>Crouch and Press the ability key to open the ejection GUI. Store up to three different projectile types in your dimension here, up to 128 per stack.
Holding the key will spawn portals. The user must charge for 3 seconds before they can fire, otherwise the portals will not be fully opened.
</p><p>Releasing the key forcefully ejects one projectile from EVERY SLOT in the GUI out at 3x speed than that of a bow would after a wind up, dealing increased damage. On mastery, you can now place 5 projectiles into your GUI.
</p><p>Will build up the counter if this is triggered. Additionally, the charge time is halved on mastery.
</p>
<ul><li>[Active, Held] Authority of the Gods:</li></ul>
<p>Crouch using will allow you to bring yourself into the Kamui dimension over a period of time, determined by your player’s current size. For reference, a 2 block tall default sized player will take 2 seconds to warp into the dimension. Releasing crouch will cancel this process.
</p><p>On mastery, double the speed that it would take to transfer you into the dimension. You can also now target other entities provided they are within your interaction range.
Will build up the counter if this is triggered.
</p>
<ul><li>[Active (Mastery), Press] Intangibility: The user becomes able to control their intangibility. On pressing the key, become intangible for 5 seconds, allowing the user to walk straight through projectiles, as well as dodge ALL attacks. Cooldown of 10 seconds after it wears off.</li></ul>
<p><br/>
</p>
<h2><span class="mw-headline" id="Version_history">Version history</span></h2>
<ul><li><b>1.21.1</b> – Added to the game</li></ul>
<ul><li><b>1.19.2</b> – Didn't exist</li></ul>




</div>
</div>

<section class="reference-related">
<div class="reference-related-heading">
<h2>Continue exploring</h2>
<a href="../../../../tensura-reference/skills/unique/">Browse all Unique Skills</a>
</div>
<div class="reference-related-grid">
<a class="reference-related-card" href="../melancholy/">
<img alt="" decoding="async" loading="lazy" src="../../../../assets/upstream/mysticism/skills/melancholy-53d9520383.png"/>
<span class="reference-related-copy">
<strong>Melancholy</strong>
<small>Summon an aura of sorrow that slows and weakens nearby enemies, filling them with despair. The skill also throws…</small>
</span>
</a>
<a class="reference-related-card" href="../malleable/">
<img alt="" decoding="async" loading="lazy" src="../../../../assets/upstream/mysticism/skills/malleable-e63a1cb2e5.png"/>
<span class="reference-related-copy">
<strong>Malleable</strong>
<small>[Sub-Skill] Cold Clay - The user will learn Flame Attack Resistance when Malleable is acquired.</small>
</span>
</a>
<a class="reference-related-card" href="../provider/">
<img alt="" decoding="async" loading="lazy" src="../../../../assets/upstream/mysticism/skills/provider-93310a03a9.png"/>
<span class="reference-related-copy">
<strong>Provider</strong>
<small>HERE, TAKE MY GLORIOUS STUFF! (I like these texts)</small>
</span>
</a>
</div>
</section>

---

## Source and licensing

TR Mysticism reference adapted from [Phaser](https://trmysticism.wiki.gg/wiki/Phaser) on the Tensura Reincarnated: Mysticism Wiki (revision `2812`, modified `2026-04-16T13:17:23Z`). Adapted text is available under [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/).

<details class="reference-media-credits">
<summary>Media credits (1 source files)</summary>
<ul>
<li><a href="https://trmysticism.wiki.gg/wiki/File:Phaser.png">Phaser.png</a> — CC BY-SA 4.0; uploaded by Hunterman4488; revision 2974</li>
</ul>
</details>
